 ...
 * Now includes a Column Manager UI for visibility control
 */
...
import { useDataTable } from "../packages/react";
import { DataTable, ColumnManager } from "../packages/ui";
...
/**
 * Basic Example Component with Column Manager
 */
export function BasicExample() {
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial, sans-serif", maxWidth: "1200px" }}>
      <h1>Basic Table Example with Column Manager</h1>

      {/* Column Manager */}
      <ColumnManager
        columns={table.columns}
        columnVisibility={table.state.columnVisibility}
        onToggleVisibility={(columnId) => table.columnManagement.toggleVisibility(columnId)}
        onSetVisibility={(columnId, visible) => table.columnManagement.setVisibility(columnId, visible)}
        onSetAllVisibility={(visibility) => table.columnManagement.setAllVisibility(visibility)}
      // Example: Customize styling
      // styles={{
      ...
      // buttonClassName="custom-column-manager-button"
      // modalClassName="custom-column-manager-modal"
      />
      ...
      >
        ...
        <span>
          Page {table.pageIndex + 1} of {table.pageCount} ({table.filteredRowCount} total items)
        </span>
        <button
      ...
      >
        ...
        <br />
        Visible Columns: {table.visibleColumns.length} / {table.columns.length}
        <br />
        Hidden Columns:{" "}
        {table.columns
          .filter((col) => table.state.columnVisibility[col.id] === false)
